...
settings: {
    ...
    */
	...
	/**
	* If you just want to submit the form in time for the cursor on the first wrong form elements, will set it to true and set focusOnerror to false;
	* @default true
	*/
	focusOnerrorSubmit: true,
    ...
    * object:{validanguage.hideError,validanguage.removeErrorTitle}
    ...
    * If you want to use only alert messages via showAlert, set this to {} to turn off inline error msgs
    * object:{validanguage.showError,validanguage.addErrorTitle}
    * @default 'validanguage.showError' 
    */
    onerror: 'validanguage.showError',
    
    /**
    * This attribute used by save the error meassage to the taget.
    * If you set showTooTip is true, cannot set it 'title'.
    * @defaut 'errorMsgAttr'
    */
    errorMsgAttr :'errorMsgAttr',
    
    /**
    * Show the error message by TooTip.
    * If you want to show the error message by TooTip set it true,and before use it, 
    * you must set onerrr:'validanguage.addErrorTitle' and onsuccess'validanguage.removeErrorTitle'.
    * when you set onerrr:'validanguage.addErrorTitle' and onsuccess'validanguage.removeErrorTitle' but showTooTip is false,
    * the error message showed by title.
    * @default true
    */
    showTooTip: true,
    
    /**
    * The ID used by show the error message in showTooTip(). The errorMsgSpanSuffix is appended to the end of the form to make the DIV.
    * If a DIV with this ID already exists in the DOM, it will be used. If it doesn't exist, one will be created dynamically.
    * @default 'tooTipDiv'
    */
    showTooTipDiv: 'tooTipDiv',
...
/**
* Generic cross-browser removeEvent() function.
*
* @param {object} Object to receive the event
* @param {Object} Event type
* @param {Object} Function to be called
...
/**
 * This function returns the className of the parent element.
 * @param (domNode) The parent domument.
 * @param (String) The class name of need to check
 * @param (boolean) If the current element division form elements, whether its as the form element processing 
 */
getElementByClassName: function(oParent,className,isForm){
    var eResult = [];
    var pattern = new RegExp("\\b"+className+"\\b");
    var i = 0;
    var oParentChild = null;
    if(isForm){
        oParentChild = oParent.elements;
    }else{
        oParentChild = oParent.getElementsByTagName("*");
    }
    for(i; i < oParentChild.length; i++){
        if (pattern.test(oParentChild[i].className)){
            eResult.push(oParentChild[i]);
        }
    }
    return eResult;
},
...
hideError: function() {
    ...
    if (! this.className.match(validanguage.settings.passedFieldClassName)) this.className = this.className.trim() + ' ' + validanguage.settings.passedFieldClassName;
...
/**
*This function remmove the target's title attribute, title attribute to keep this error message. 
*/
removeErrorTiTle: function(){
    var settings = validanguage.getFormSettings(this.id);
    this.setAttribute(settings.errorMsgAttr,'');
    if(! this.className.match(validanguage.settings.passedFieldClassName)) this.className =this.className.trim() + ' ' + validanguage.settings.passedFieldClassName;
    if(this.className.match(validanguage.settings.failedFieldClassName)) this.className = this.className.replace(validanguage.settings.failedFieldClassName,'');
    if( !settings.showFailedFields) return;
    if(document.getElementById(this.id + settings.errorListItemSuffix) != null ){
...
showError: function( errorMsg ) {
    ...
    if (!this.type || (this.type != 'radio' && this.type != 'checkbox')) {
        if (!this.className.match(validanguage.settings.failedFieldClassName)) 
            this.className =this.className.trim() + ' ' + validanguage.settings.failedFieldClassName;
    ...
    var errorList = document.getElementById(settings.errorListId);
    var listItem = '<li id="'+this.id+settings.errorListItemSuffix+'"><label for="'+this.id+'">'+validanguage.el[this.id].field+'</label></li>';
...
/**
* This function is add the error message to the title attribute, and change the class to settrings.failedFieldClassName
* @param {String} Text of the error message to be add the title attribute
*/
addErrorTitle: function ( errorMsg){
    var settings = validanguage.getFormSettings(this.id);
    this.setAttribute(settings.errorMsgAttr,errorMsg);
    if (!this.type || (this.type != 'radio' && this.type != 'checkbox')) {
        if (!this.className.match(validanguage.settings.failedFieldClassName)) 
            this.className =this.className.trim() + ' ' + validanguage.settings.failedFieldClassName;
        if (this.className.match(validanguage.settings.passedFieldClassName)) 
    ...
    validanguage.validateTooTip(this, settings);
    if( !settings.showFailedFields ) return;
    if( document.getElementById(settings.errorDivId) == null ) {
        var errorDiv = document.createElement('DIV');
        errorDiv.id = settings.errorDivId;
    ...
    if (document.getElementById(settings.errorListId) == null) {
        errorDiv.innerHTML = settings.errorListText + '<ul id="'+settings.errorListId+'"></ul>';
    }
    var errorDivInner = errorDiv.innerHTML.toLowerCase();
    errorDivInner = errorDivInner.replace(/"/g,''); //remove quotes for IE weirdness
...
/**
* This function create DIV of TooTip and append the error message to DIV
* If you use a tag that the form cannt to get data from the tag like "DIV"
* 
* @param {innerHTMl Ojbect} 
* @param {validanguage Object} can be null
*/
validateTooTip: function(obj, settings){
    if( settings == undefined || settings == ' ') settings = validanguage.settings;
    if( !validanguage.settings.showTooTip) return;
    var tooTipDiv = document.getElementById(settings.showTooTipDiv);
    if(tooTipDiv == null){
        tooTipDiv = document.createElement('DIV');
        tooTipDiv.id = settings.showTooTipDiv;
        document.body.appendChild(tooTipDiv);
        tooTipDiv.style.display = 'none';
    }
    obj.onmouseover = function(e){
        var errorMsg = this.getAttribute(validanguage.settings.errorMsgAttr);
        if((this.className).indexOf (validanguage.settings.failedFieldClassName) == -1) return; 
        var winHeight = document.documentElement.clientHeight;
        var winWidth = document.documentElement.clientWidth;
        var bottomY = winHeight -(e.pageY - document.documentElement.scrollTop);
        var rightX = winWidth - (e.pageX - document.documentElement.scrollLeft);
        var tooltipHeight = tooTipDiv.style.height;
        var tooltipWidth = tooTipDiv.style.width;
        //height of tooltip div
        var tth = tooltipHeight.substring(0,tooltipHeight.length-2);
        var ttw = tooltipWidth.substring(0,tooltipWidth.length-2);
        //Automatically determine the direction
        if(bottomY >= tth){
            tooTipDiv.style.top = (e.pageY+20)+"px";
        }else{
            tooTipDiv.style.top = (e.pageY-tth-20)+"px";
        }
        if(rightX >= ttw){
            tooTipDiv.style.left =(e.pageX+20)+"px";
        }else{
            tooTipDiv.style.left =(e.pageX-ttw-20)+"px";
        }
        var innerHTML = '<div class="'+settings.showTooTipDiv+'_img"></div>'
                      + '<div class="'+settings.showTooTipDiv+'_title" ><span>'+errorMsg+'</span></div>';
        tooTipDiv.style.display = 'block';
        this.setAttribute(validanguage.settings.errorMsgAttr, '');
        tooTipDiv.innerHTML = innerHTML;
    };
    obj.onmouseout = function(e){
        tooTipDiv.style.display = 'none';
        if(tooTipDiv.hasChildNodes() && ((obj.className).indexOf(validanguage.settings.failedFieldClassName) != -1)) {
            for(var i=0; i<tooTipDiv.childNodes.length; i++) { 
                var node = tooTipDiv.childNodes[i]; 
                if(node.nodeType == 3 && !/\S/.test(node.nodeValue)) { 
                    node.parentNode.removeChild(node);  
                } 
            }
            obj.setAttribute(validanguage.settings.errorMsgAttr, tooTipDiv.lastChild.innerText);
        }else obj.setAttribute(validanguage.settings.errorMsgAttr, '');
    };
},//close validateTooTip
...
validateRequired: function( unused ) {
    ...
    for( var i=alternatives.length-1; i>-1; i--) {
        ...
        var text = elem.value.replace(/(^\s*)|(\s*$)/g, "");
    ...
    if(type=='submit') {
        ...
        if( validanguage.empty(failedValidations) ) {
           ...
           for (var o in failedValidations) {
            ...
            var focusOnerrorSumbit = validanguage.getElSetting("focusOnerrorSubmit", id, validation);
            var formDom = document.forms[form];
            var aResult =  validanguage.getElementByClassName(formDom, validanguage.settings.failedFieldClassName, false);
            if(aResult[0]){
               aResult[0].focus();
            }
...
/**
 * This function to support a way that do not use submit-button to submit a form when you add submit event.
 * @param {Object} formId
 */
validateSubmit: function(formId){
    var form = document.getElementById(formId);
    if( form == undefined) return;
    var eventObj = document.createEvent('HTMLEvents'); 
    eventObj.initEvent('submit', false, true);
    form.dispatchEvent(eventObj);
    var failObject = validanguage.getElementByClassName(form, validanguage.settings.failedFieldClassName, false);
    if( typeof failObject == 'object' && failObject[0] == undefined ) form.submit();
}
